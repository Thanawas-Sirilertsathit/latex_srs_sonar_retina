...
For the SSL service, we implemented a \textit{Generalized Cross-Correlation with Phase Transform} (GCC-PHAT) model. While traditionally an algorithm, it is utilized here as a classification model to estimate the proximity and direction of sound sources from the equation below.

\begin{equation}
    R_{x_1x_2}^{PHAT}(\tau) = \int_{-\infty}^{\infty} \frac{X_1(f) X_2^*(f)}{|X_1(f) X_2^*(f)|} e^{j2\pi f\tau} df
\end{equation}

\textbf{Design Decision:} The PHAT weighting functions as a whitening filter, effectively stripping away the magnitude information and focusing solely on the phase difference. 
This makes the model highly resilient to varying volumes and background noise levels. To minimize "Time-to-Insight" for the user, the model's raw Time Difference of Arrival (TDOA) outputs are mapped into five discrete distance categories: \textit{Very Close, Close, Medium, Far,} and \textit{Very Far}. 
This mapping is calibrated using benchmarks from the DCASE dataset to ensure consistency across different mobile hardware configurations.
